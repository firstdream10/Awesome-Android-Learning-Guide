...
$$
其中 $h(key)$ 就是这里所说的**散列函数**，对于每个关键码为 $key$ 的元素而言，其通过散列函数计算出散列地址 $h(key)$，然后将 $value$ 存储在散列地址上。
如果两个不相等的关键码 $key1$ 和 $key2$，通过散列函数计算之后得到了相同的散列地址（即 $h(key1) = h(key2)$），这种现象称之为**碰撞**，而发生碰撞的两个关键码称之为**同义词**

散列函数 $h(key)$ 的定义域是整个关键码的集合，$h(key)$ 的值域应该是可以使用的整个地址空间，称之为**基本区域**。为了评价散列函数的好坏以及衡量检索的效率还引入了以下两个概念
**负载因子（$a$）**:
$$
a = {关键码的节点数\over 基本区域能容纳的节点数}
$$
当 $a>1$ 时必定会出现碰撞

**平均检索长度（$ASL$）**：
$$
ASL(n)=\sum_{i=1}^{n}p_{i}c_{i}
$$
其中 $n$ 是 $Hash$ 表中的元素个数，$p_{i}$ 是查找第 $i$ 个元素的概率，$c_{i}$ 是找到第 $i$ 个元素的比较次数。$ASL$ 越小代表性能越好
...
- 访问速度很快
  ...
  $Hash$ 其实是随机存储的一种优化，先进行分类，然后查找时按照这个对象的分类去找。$Hash$ 通过一次计算大幅度缩小查找范围，自然比从全部数据里查找速度要快。理想情况下 $Hash$ 表检索的时间复杂度为 $O(1)$，而顺序检索的时间复杂度为 $O(n)$
...
5. 除余法
   ...
   选择一个适当的正整数 $P$，用 $P$ 去除关键码，余数作为散列地址，即 $h(key) = key\%P$ 。
   这个方法的关键在于选取适当的 $P$。如果 $P$ 为偶数，则总是把奇数的关键码转换为奇数地址，把偶数的关键码转换为偶数地址；如果 $P$ 是关键码的基数的幂次，则相当于选择关键码的最后几位数字为地址。这两种情况都不太好。一般 $P$ 为不大于基本区域长度的最大素数比较好。
...
2. 双散列函数法（$d_{i}=i*h_{2}(key)$）

   在上述例子中改用双散列函数法插入19，其中 $h_{2}(key)=key\%7+1$

   - $h(19)=6$，发生碰撞
   - $H_{1} = (6 + (19 \% 7 + 1)) \%13 = 12$，未发生碰撞，存储在12
...
在没有发生碰撞的情况下， $Hash$ 表是在查找中效率最高的，此时它的时间复杂度为$O(1)$。然而在实际应用中，碰撞是不可避免的，所以 $Hash$ 表的效率取决于碰撞发生的次数。影响碰撞的因素主要有以下三点：

1. 散列函数计算出散列地址是否均匀，越均匀发生碰撞的概率越低
2. 处理冲突的方法：相同的关键字，相同的散列函数，处理冲突的方法不同，会使得 $ASL$ 不同。
3. 负载因子$a$：当填入表中的记录越多，$a$越大，产生冲突的可能性就越大，所以这个时候需要用扩容解决负债因子过大的问题
